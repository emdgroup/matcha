"""GT3D pretraining model for supervised multi-task learning on 3D graphs.

Jointly predicts user-provided atom-level and molecule-level labels via a
shared 3D-aware Graph Transformer encoder with separate prediction heads.
Consumes batches produced by :class:`Graph3DPretrainingDataModule` — 3D
coordinates ride on ``graph.pos`` and are read by the encoder's per-layer
hook.
"""
...
class GT3DPretraining(BaseGraphPretrainingModel):
    """GT3D model for pretraining with node-level and graph-level predictions.
    ...
    encoder's :meth:`GT3D.forward_nodes_per_layer` reads them from there.

    Example usage:

    .. code-block:: python

        model = GT3DPretraining(
    ...
    :param int enc_num_layers: Number of GT3D transformer layers, defaults to 3
    ...
    def _build_encoder(self):
        """Build the canonical GT3D encoder shared with the classic path."""
        atom_input_dim = (
