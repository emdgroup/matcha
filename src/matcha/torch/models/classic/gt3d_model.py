...
from matcha.utils.schemas import GT3DInputModel, UncertaintyMethod


@ClassicModelRegistry.register()
class GT3DModel(BaseClassicModel, HyperparametersMixin):
    """Graph Transformer with 3-D distance kernels (GT3D) for molecular property
    prediction from conformers.

    Extends the GT architecture with Gaussian distance kernels to incorporate
    3-D structural information. Loosely inspired by the gt-pyg implementation.
    ...
    def __init__(
        ...
        self.params = GT3DInputModel(**self.hparams)
        atom_input_dim = (
            enc_atom_input_dim
            + enc_laplacian_k
            + enc_rwse_k
