"""Tests for :class:`matcha.torch.models.pretraining.gps3d_pretraining.GPS3DPretraining`.

Locks in the issue #34 contract for GPS3D:

- the pretraining model wires up the canonical :class:`GPS3D` encoder rather
...
- encoder weights transfer cleanly into a matching :class:`GPS3DModel` for
...
def _make_model(**overrides) -> GPS3DPretraining:
    """Build a tiny GPS3DPretraining with positional encodings disabled."""
    kwargs = dict(
        num_node_targets=2,
        num_graph_targets=1,
    ...
    return GPS3DPretraining(**kwargs)
...
def test_encoder_is_canonical_gps3d():
    """The pretraining model instantiates the canonical :class:`GPS3D` encoder."""
    model = _make_model()
    assert isinstance(model.encoder, GPS3D)


def test_registered_on_pretraining_registry():
    """``GPS3DPretraining`` is discoverable via the pretraining registry."""
    assert "gps3dpretraining" in PretrainingModelRegistry
    assert PretrainingModelRegistry["gps3dpretraining"] is GPS3DPretraining
...
def _silence_lightning_log(model: GPS3DPretraining) -> None:
...
def test_save_hyperparameters_round_trip_through_state_dict():
    ...
    fresh = GPS3DPretraining(**hparams)
...
def test_encoder_weights_transfer_to_gps3d_model():
    """Encoder weights from a pretrained model load cleanly into ``GPS3DModel``.

    Headline acceptance criterion for issue #34: after a pretraining run,
    users must be able to hand the encoder weights to :class:`GPS3DModel`
