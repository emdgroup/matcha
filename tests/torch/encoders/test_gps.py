"""Tests for :class:`matcha.torch.encoders.gps.GPS`.
...
def _make_gps(**overrides) -> GPS:
    """Build a tiny GPS encoder with all positional encodings disabled."""
    kwargs = dict(
...
def test_forward_uses_template_from_base_graph_encoder():
    ...
    verifies that :class:`GPS` inherits it (i.e. did not shadow it) and that
    the layer loop still drives the same output the classic path sees.
    """
    torch.manual_seed(0)
    encoder = _make_gps(jk="concat", readout="sum")
    encoder.eval()
...
def test_forward_output_shape_matches_jk(jk: str):
    torch.manual_seed(0)
    encoder = _make_gps(jk=jk, readout="sum")
    encoder.eval()
