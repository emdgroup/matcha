"""Tests for :class:`matcha.torch.encoders.gatedgcn.GatedGCN`.
...
def _make_gatedgcn(**overrides) -> GatedGCN:
    """Build a tiny GatedGCN encoder with all positional encodings disabled."""
    kwargs = dict(
...
def test_forward_uses_template_from_base_graph_encoder():
    ...
    verifies that :class:`GatedGCN` inherits it (i.e. did not shadow it) and
    that the layer loop still drives the same output the classic path sees.
    """
    torch.manual_seed(0)
    encoder = _make_gatedgcn(jk="concat", readout="sum")
    encoder.eval()
...
def test_forward_output_shape_matches_jk(jk: str):
    torch.manual_seed(0)
    encoder = _make_gatedgcn(jk=jk, readout="sum")
    encoder.eval()
