"""Tests for :class:`matcha.torch.encoders.e3gnn.E3GNN`.

Pins the unified base-encoder contract (issue #24 + #26):
:meth:`forward_nodes_per_layer` produces the per-layer node embeddings
consumed by both the classic and pretraining paths;
:meth:`BaseGraphEncoder.forward` is a thin template that combines those
embeddings via jumping knowledge and the configured readout; and E3GNN
reads 3D coordinates from ``graph.pos`` rather than a separate positional
argument.
"""
...
def _make_e3gnn(**overrides) -> E3GNN:
    """Build a tiny E3GNN encoder with all positional encodings disabled."""
    kwargs = dict(
...
def _make_batch(batch_size: int = 2, with_pos: bool = True) -> Batch:
    """Small PyG batch of chain graphs with random node/edge features + coords."""
    graphs = []
    for _ in range(batch_size):
        ...
        if with_pos:
            kwargs["pos"] = torch.randn(n_nodes, 3)
...
def test_forward_uses_template_from_base_graph_encoder():
    ...
    Verifies that :class:`E3GNN` inherits the base template method and did
    not shadow it, so the classic path and the pretraining hook see the
    same layer stack.
    """
    torch.manual_seed(0)
    encoder = _make_e3gnn(jk="concat", readout="sum")
    encoder.eval()
...
def test_forward_output_shape_matches_jk(jk: str):
    torch.manual_seed(0)
    encoder = _make_e3gnn(jk=jk, readout="sum")
    encoder.eval()
...
def test_missing_pos_raises_clear_error():
    """Batch without ``pos`` must fail loudly rather than silently produce
    garbage — 3D encoders require the datamodule to attach coordinates.
    """
    torch.manual_seed(0)
    encoder = _make_e3gnn()
    encoder.eval()

    batch = _make_batch(with_pos=False)
