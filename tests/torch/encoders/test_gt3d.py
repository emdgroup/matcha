"""Tests for :class:`matcha.torch.encoders.gt3d.GT3D`.
...
embeddings via jumping knowledge and the configured readout; and GT3D
...
def _make_gt3d(**overrides) -> GT3D:
    """Build a tiny GT3D encoder with all positional encodings disabled."""
    kwargs = dict(
...
def test_forward_uses_template_from_base_graph_encoder():
    torch.manual_seed(0)
    encoder = _make_gt3d(jk="concat", readout="sum")
    encoder.eval()
...
def test_forward_output_shape_matches_jk(jk: str):
    torch.manual_seed(0)
    encoder = _make_gt3d(jk=jk, readout="sum")
    encoder.eval()
