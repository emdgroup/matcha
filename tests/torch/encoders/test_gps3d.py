"""Tests for :class:`matcha.torch.encoders.gps3d.GPS3D`.
...
embeddings via jumping knowledge and the configured readout; and GPS3D
...
def _make_gps3d(**overrides) -> GPS3D:
    """Build a tiny GPS3D encoder with all positional encodings disabled."""
    kwargs = dict(
...
def test_forward_uses_template_from_base_graph_encoder():
    torch.manual_seed(0)
    encoder = _make_gps3d(jk="concat", readout="sum")
    encoder.eval()
...
def test_forward_output_shape_matches_jk(jk: str):
    torch.manual_seed(0)
    encoder = _make_gps3d(jk=jk, readout="sum")
    encoder.eval()
